...
Creates:
    ...
    cache/perch/arrays.npz        scores[N,C] + emb[N,1536] float32
    cache/perch/labels.npy        multi-hot Y[N,C] uint8 (labeled rows only; unlabeled rows all-zero)
    cache/perch/proxy_map.json    unmapped-species genus proxy indices
...
def main(dry_run_files: int = 0) -> None:
    ...
    np.savez_compressed(PERCH_NPZ, scores_full_raw=scores, emb_full=embs)
