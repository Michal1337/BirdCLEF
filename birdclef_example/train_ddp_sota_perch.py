...
class PerchSpectrogramClassifier(nn.Module):
    def __init__(
        self,
        n_classes: int,
        backbone_path: str,
        ...
        # Load the converted Perch backbone (EfficientNet-B3 feature extractor).
        ...
        if feature_dim is None:
            raise RuntimeError("Unable to infer feature dimension from Perch backbone.")
        self.dropout = nn.Dropout(dropout)
        self.head = nn.Linear(int(feature_dim), n_classes)
...
def build_optimizer(
    ...
    param_groups = [
        ...
        {"params": backbone_params, "lr": 0.0, "weight_decay": weight_decay},
    ]
...
def build_experiment_configs() -> List[Dict[str, Any]]:
    ...
    for lr_backbone in [6e-5, 8e-5, 1.0e-4, 1.2e-4]:
        for lr_head in [6e-4, 7e-4, 8e-4, 9e-4]:
            ...
            exp.update(
                {
                    "name": f"sota{idx:02d}_perch_lr{lr_head:.0e}_{lr_backbone:.0e}",
                    "lr_head": lr_head,
    ...
    for mixup_alpha, smoothing in [(0.10, 0.02), (0.15, 0.02), (0.25, 0.02), (0.25, 0.03)]:
        ...
        exp.update(
            {
                "name": f"sota{idx:02d}_perch_mix{int(mixup_alpha*100)}_ls{int(smoothing*100)}",
                "lr_head": 8e-4,
                "lr_backbone": 1.0e-4,
    ...
    for focal_gamma, focal_weight in [(1.5, 0.25), (2.0, 0.35), (2.5, 0.35)]:
        ...
        exp.update(
            {
                "name": f"sota{idx:02d}_perch_fg{str(focal_gamma).replace('.', '')}_fw{int(focal_weight*100)}",
                "lr_head": 8e-4,
                "lr_backbone": 1.0e-4,
                "mixup_alpha": 0.2,
                "label_smoothing": 0.03,
...
def run_experiment(
    ...
    model_config = {
        ...
        "backbone_path": str(base_model_config["backbone_path"]),
    ...
    model = PerchSpectrogramClassifier(**model_config).to(device)
    ...
    _print_main(rank, f"Backbone: {base_model_config['backbone_path']} | image_size={experiment['image_size']}")
    ...
    for epoch in range(1, epochs + 1):
        ...
        if _is_main_process(rank):
            ...
            row = {
                ...
                "val_loss": val_loss,
                "val_auc": val_auc,
                "backbone_name": "perch_backbone.pt",
                "mixup_alpha": float(experiment["mixup_alpha"]),
                "mixup_probability": float(experiment["mixup_probability"]),
                "bce_weight": float(experiment["bce_weight"]),
                "focal_weight": float(experiment["focal_weight"]),
    ...
    summary: Dict[str, Any] = {
        "experiment": exp_name,
        "architecture": "sota_perch_spectrogram",
        "backbone_name": "perch_backbone.pt",
...
def main() -> None:
    ...
    repo_root = Path(__file__).resolve().parents[1]
    data_dir = Path("/mnt/evafs/groups/re-com/mgromadzki/data")
    output_dir = repo_root / "birdclef_example" / "outputs" / "experiments_sota_perch"
    backbone_path = repo_root / "perch_backbone.pt"
