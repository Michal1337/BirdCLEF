...
def main() -> None:
    ...
    pseudo_labeling_v1_enabled = True
    pseudo_labels_path = output_dir / "pseudo_labels_v1.csv"
    ...
    pseudo_max_labels_per_clip = 3
    pseudo_top1_fallback_threshold = 0.75
    pseudo_mix_ratio = 0.20
    ...
    if pseudo_labeling_v1_enabled and pseudo_labels_path.exists():
        try:
    ...
    _print_main(rank, f"Pseudo-label v1 enabled: {pseudo_labeling_v1_enabled}")
    ...
    if can_run_pseudo_only:
        _print_main(
            ...
            "running pseudo-label generation only (skipping fold retraining).",
        )
        ...
        if _is_main_process(rank):
            ...
            print(f"Pseudo-label v1 generated: {len(pseudo_df)} rows at {pseudo_labels_path}")
        return
    ...
    if _is_main_process(rank):
        ...
        if pseudo_labeling_v1_enabled and not pseudo_labels_ready:
            ...
            pseudo_df.to_csv(pseudo_labels_path, index=False)
            print(
                f"Pseudo-label v1 generated: {len(pseudo_df)} rows at {pseudo_labels_path}. "
                "Re-run training to consume pseudo labels."
            )
