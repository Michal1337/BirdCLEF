...
Variant: uses the fold's validation split directly for probe evaluation (no internal train/val split).

This script is a variant of probe_config_sweep.py, but in train_torch_probes_nosplit(),
the probe is trained on the entire fold's train split and evaluated on the fold's validation split only.
"""
...
# Variant: train_torch_probes_nosplit trains on all train data, no internal val split
def train_torch_probes_nosplit(
    emb: np.ndarray,
    ...
    for ci in active:
        ...
        # No internal val split: train on all x_bal/y_bal
        if standardize_features:
            mu = x_bal.mean(axis=0, keepdims=True)
            sd = x_bal.std(axis=0, keepdims=True)
...
def run_oof_probe_config(
    ...
    for fold, (tr_idx, va_idx) in enumerate(gkf.split(scores_raw, groups=groups), 1):
        ...
        # Use the variant: train on all tr, evaluate on va
        probe_models, emb_scaler, emb_pca = train_torch_probes_nosplit(
            emb_tr,
            sc_tr,
            y_tr,
            cfg,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Probe OOF config sweep (no internal val split)")
    parser.add_argument(
        "--cache-meta",
        type=Path,
    ...
    parser.add_argument("--out-dir", type=Path, default=Path("outputs/probe_sweep_nosplit"))
