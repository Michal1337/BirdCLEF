...
def _prime_pretrained_cache(model_id: str, n_classes: int, rank: int) -> None:
    # Avoid all ranks hammering HF hub at once.
    if _is_main_process(rank):
...
class HFAudioPretrainedClassifier(nn.Module):
    def __init__(
        self,
        n_classes: int,
        model_id: str,
    ...
    def _wave_to_model_features(self, waveform: torch.Tensor) -> torch.Tensor:
        # B,1,T -> B,1,M,F
        spec = self.spectrogram(waveform)
        if self.training:
            spec = self.spec_augment(spec)

        # AST-family models expect mel-bins x frames, typically 128 x ~1024.
        spec = F.interpolate(
            spec,
            size=(spec.size(2), self.target_frames),
            mode="bilinear",
            align_corners=False,
        )

        # B,1,M,F -> B,F,M
        features = spec.squeeze(1).transpose(1, 2)

        # HF AST docs recommend mean ~0 and std ~0.5 at input.
        features = features * 0.5
        return features

    def forward(self, waveform: torch.Tensor) -> torch.Tensor:
        input_values = self._wave_to_model_features(waveform)
...
def build_experiment_configs() -> List[Dict[str, Any]]:
    # Focused sweep around the strongest setup observed so far:
    # MIT/ast-finetuned-audioset-10-10-0.4593 with no freeze, light aug, low backbone LR.
    return [
        {
            "name": "aft01_focus_anchor",
        ...
        {
            "name": "aft09_very_light_specaugment",
            "model_id": "MIT/ast-finetuned-audioset-10-10-0.4593",
            "n_mels": 128,
            "target_frames": 1024,
            "freq_mask_param": 6,
...
def run_experiment(
    ...
    _prime_pretrained_cache(model_id=model_id, n_classes=len(label_map), rank=rank)
    ...
    model_config.update(
        {
            ...
            "target_frames": int(experiment["target_frames"]),
    ...
    model = HFAudioPretrainedClassifier(**model_config).to(device)
    ...
    _print_main(rank, f"Pretrained model: {model_id}")
    ...
    summary: Dict[str, Any] = {
        "experiment": exp_name,
        "architecture": "hf_audio_pretrained",
        "model_id": model_id,
...
def main() -> None:
    ...
    output_dir = repo_root / "birdclef_example" / "outputs" / "experiments_ft_audio2"
        ...
        try:
            ...
            if _is_main_process(rank):
                failure = {
                    ...
                    "architecture": "hf_audio_pretrained",
                    "model_id": experiment.get("model_id", "unknown"),
                    "status": "failed",
                    "error": str(ex),
                }
