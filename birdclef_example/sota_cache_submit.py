...
def load_finetuned_head(checkpoint_path: Path, n_classes: int):
...
def run_perch(paths, batch_files=16, verbose=True):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=4) as io_executor:
        ...
        for start in itr:
            ...
            if len(NO_SIGNAL_POS) > 0:
                with torch.no_grad():
                    spatial_t = torch.from_numpy(spatial_emb).float()
                    finetuned_logits = FINETUNED_HEAD(spatial_t).cpu().numpy().astype(np.float32)
                if finetuned_logits.shape[1] != len(NO_SIGNAL_POS):
                    raise ValueError(
                        f"Finetuned logits dim mismatch: got {finetuned_logits.shape[1]}, expected {len(NO_SIGNAL_POS)}"
                    )
                scores[br:wr, NO_SIGNAL_POS] = finetuned_logits
...
MODE = "submit"  # default for submission
CFG = {
    "name": "low_prior_hrm_blend_f_submit",
    # inference
    "batch_files": 16,
    # local CV
    "oof_n_splits": 5 if MODE == "train" else 3,
    # full-stack submit defaults (from top sweep2 config family)
    "ensemble_w": 0.45,
...
FINETUNED_CKPT = BASE / "outputs" / "experiments_ft" / "no_stage2_cfg2.pt"
...
print(
    f"  preset={CFG['name']}  "
    f"prior={float(CFG.get('lambda_prior', 0.4)):.2f}  "
    f"ensemble_w={float(CFG.get('ensemble_w', 0.5)):.2f}  "
    f"mlp_blend={float(CFG.get('mlp_alpha_blend', 0.4)):.2f}"
)
print("Config ready")
print(
...
NO_SIGNAL_POS = np.array([int(idx) for idx in UNMAPPED_POS if int(idx) not in proxy_map], dtype=np.int32)
if not FINETUNED_CKPT.exists():
    raise FileNotFoundError(f"Missing finetuned checkpoint: {FINETUNED_CKPT}")
FINETUNED_HEAD = load_finetuned_head(FINETUNED_CKPT, n_classes=len(NO_SIGNAL_POS))
print(f"Unmapped species total:        {len(UNMAPPED_POS)}")
print(f"Species with genus proxy:      {len(proxy_map)}")
print(f"Species covered by finetuned head: {len(NO_SIGNAL_POS)}")
print("\nProxy targets:")
...
if CFG["run_oof"]:
    ...
    baseline_auc, oof_raw = honest_oof_auc(
        sc_tr, Y_FULL_aligned, meta_tr, n_splits=CFG["oof_n_splits"], label="raw Perch"
    )
...
if not test_paths:
    ...
    test_paths = sorted(Path("data/train_soundscapes").glob("*.ogg"))[:n]
else:
...
proto_model, site2i_tr = train_light_proto_ssm(
    ...
    Y_FULL_aligned,
    meta_tr,
    n_epochs=int(CFG.get("proto_n_epochs", 40)),
    patience=int(CFG.get("proto_patience", 8)),
    lr=float(CFG.get("proto_lr", 1e-3)),
...
test_site_ids = np.array(
    [
        min(
            ...
            int(CFG.get("proto_n_sites", 20)) - 1,
        )
...
sc_te_adjusted = apply_prior(
    ...
    tables=prior_tables,
    lambda_prior=float(CFG.get("lambda_prior", 0.4)),
)
probe_models, emb_scaler, emb_pca, alpha_blend = train_mlp_probes(
    emb=emb_tr,
    scores_raw=sc_tr,
    Y=Y_FULL_aligned,
    min_pos=int(CFG.get("mlp_min_pos", 5)),
    pca_dim=int(CFG.get("mlp_pca_dim", 64)),
    alpha_blend=float(CFG.get("mlp_alpha_blend", 0.4)),
)
...
first_pass_flat = float(CFG.get("ensemble_w", 0.5)) * proto_scores_flat + (
    1.0 - float(CFG.get("ensemble_w", 0.5))
) * sc_te_adjusted
...
proto_tr_out = run_tta_proto(
    ...
    shifts=[int(x) for x in CFG.get("tta_shifts", [0, 1, -1, 2, -2])],
)
...
first_pass_tr = float(CFG.get("ensemble_w", 0.5)) * proto_tr_flat + (
    1.0 - float(CFG.get("ensemble_w", 0.5))
) * sc_tr_mlp
...
res_model, correction_weight = train_residual_ssm(
    ...
    n_epochs=int(CFG.get("residual_n_epochs", 30)),
    patience=int(CFG.get("residual_patience", 8)),
    lr=float(CFG.get("residual_lr", 1e-3)),
...
probs = rank_aware_scaling(
    probs,
    n_windows=N_WINDOWS,
    power=float(CFG.get("rank_power", 0.4)),
)
probs = adaptive_delta_smooth(
    probs,
    n_windows=N_WINDOWS,
    base_alpha=float(CFG.get("smooth_alpha", 0.20)),
)
